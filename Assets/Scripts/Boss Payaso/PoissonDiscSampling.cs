...
public static class PoissonDiscSampling
{
    ...
    public static List<Vector2> GeneratePoints(float radius, Vector2 regionSize, int numSamplesBeforeRejecction)
    {
        ...
        while(spawnPoints.Count > 0)
        {
            ...
            for (int i=0; i< numSamplesBeforeRejecction; i++)
            {
    ...
    static bool IsValid(Vector2 candidate, Vector2 regionSize, float cellSize,float radius, List<Vector2> points, int[,] grid)
    {
        if (candidate.x >= 0 && candidate.x <regionSize.x && candidate.y >= 0 && candidate.y < regionSize.y)
        {
            ...
            int searchEndX = Mathf.Min(0, cellX + 2, grid.GetLength(0)-1);

            int searchStartY = Mathf.Max(0, cellY - 2);
            int searchEndY = Mathf.Min(0, cellY + 2, grid.GetLength(1) - 1);
